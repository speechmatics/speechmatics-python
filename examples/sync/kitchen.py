...
def pretty_print_search_results(rows, is_full):
    ...
    def format_timestamp(timestamp):
        ...
        return f"{TIME_FOREGROUND}{minutes:02.0f}:{seconds:04.1f}{RESET}"
...
def main():
    ...
    if not args.no_sync:
        sync.sync(args.audio_root, client, config, database)

    if args.all:
        rows = sync.fetch_all(database, args.audio_root)
        print_raw(rows)
    if args.search:
