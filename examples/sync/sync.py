...
def sync(
    ...
    db.start_new_batch(paths)

    unseen_paths = list(db.get_unseen_paths())
    if unseen_paths:
        log(f"{len(unseen_paths)} unseen path(s) to checksum...")
        for path in unseen_paths:
            log(f"  checksuming {path}")
            checksum = calculate_checksum(path)
            db.add_audio(path, checksum)
        db.conn.commit()

    untranscribed_paths = list(sorted(db.get_untranscribed_paths(transcription_config)))
...
def fetch_all(db, audio_root):
    yield from db.conn.execute(
        """
...
def search(db, audio_root, search_query):
