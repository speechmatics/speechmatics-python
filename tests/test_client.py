...
def test_handlers_called(mock_server, mocker):
    ws_client, transcription_config, audio_settings = default_ws_client_setup(mock_server.url)
...
def test_middlewares_called(mock_server, mocker):
    ...
    ws_client.add_middleware(ClientMessageType.StartRecognition, language_changing_middleware)
    ...
    client_message_counts = Counter(msg["message"] if isinstance(msg, dict) else "AddAudio"
    ...
    assert mock_server.find_start_recognition_message()["transcription_config"]["language"] == "ja"
    # The 'all' handler should have been called for every message.
...
def test_update_transcription_config_sends_set_recognition_config(mock_server):
    ...
    set_recognition_config_msgs = mock_server.find_messages_by_type("SetRecognitionConfig")
...
def test_helpful_error_message_received_on_connection_reset_error():
    ...
    with patch('websockets.connect', mock_connect):
        with patch.object(client.LOGGER, 'error', mock_logger_error_method):
            ...
            assert "Caught ConnectionResetError when attempting to connect to server" \
...
async def test__buffer_semaphore():
    ...
    buffer_size = 123
    ws_client = client.WebsocketClient(ConnectionSettings(url='fake url',
...
async def test__producer_happy_path(mocker):
    ...
    mock_read_in_chunks = mocker.patch(
        'speechmatics.client.read_in_chunks',
        new=async_iter_mock
    )
    ...
    for index, value in enumerate(msgs_states):
        ...
        if index < exp_iters - 1:
            ...
            cmp_dicts(original_state,
                      state,
                      exp_diffs={'seq_no': exp_current_seq_no})
        else:
            assert msg == json.dumps({"message": "EndOfStream",
                                      "last_seq_no": exp_final_seq_no})
            cmp_dicts(original_state,
                      state,
...
async def test__producer_semaphore_timeout(mocker):
    ...
    buffer_size = no_chunks_to_send - 1
    quick_timeout = 0.1
    ws_client = client.WebsocketClient(ConnectionSettings(
        url='fake url',
        message_buffer_size=buffer_size,
