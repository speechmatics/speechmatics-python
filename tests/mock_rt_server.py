...
class MockRealtimeLogbook:
    """
    Contains information about what happened during an active session of the mock
    realtime server. This allows tests to make assertions about what should have
    happened, e.g. what messages should have been received.
    """
    ...
    def find_messages_by_type(self, msg_name):
        ...
        """
        return [msg for msg in self.messages_received
    ...
    def find_sent_messages_by_type(self, msg_name):
        ...
        Returns:
            List[dict]: The matching list of messages.
        """
        return [msg for msg in self.messages_sent
                if isinstance(msg, dict) and msg["message"] == msg_name]
    ...
    def wait_for_clean_disconnects(self, num_disconnects=1, timeout=15):
        ...
        This is a convenience for unit-tests which may need to wait until all connections
        have been closed cleanly.

        Args:
            num_disconnects (int, optional): Target number of disconnects to wait for.
            timeout (int, optional): Number of seconds to timeout after.

        Raises:
            TimeoutError: If we have been waiting longer than the given timeout value.
        """
        start = time.time()
        while True:
...
class MockRealtimeServer(WebSocket):
    ...
    def handleMessage(self):
        ...
        try:
            # This whole block is wrapped in a try/except because the default behaviour of
            # the SimpleWebSocketServer library is to silently discard any exceptions raised
            # by these handlers. This is very unhelpful. A workaround is to catch and log
            # any exceptions explicitly here.
    ...
    def get_responses(self, message, is_binary=False):
        """
        Optionally creates a response to the given message from the client. Either returns
        a dictionary with the response message or `None` if no response should be sent.

        Args:
            message (Union[dict, bytearray]): The message received from the client. Assumes
                that if the message is a standard JSON message it has already been parsed
                into a dictionary. AddAudio messages are expected to be binary bytearrays.
            is_binary (boolean, optional): Whether or not the message is binary, implying
                that the message is an AddAudio message.
        ...
        if is_binary:
            ...
            responses.append({"message": "AudioAdded", "seq_no": self.next_audio_seq_no})
            self.next_audio_seq_no += 1
            ...
            if msg_name == "StartRecognition":
                responses.append({"message": "RecognitionStarted",
