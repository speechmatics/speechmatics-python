...
class IntegrationTests(unittest.TestCase):
    # These tests can fail when the rt server has only one worker, because one test can end
    # and another immediately starts before the worker is ready. So we retry them with a small
    # delay between attempts to avoid this.
    standard_retry = retry(wait=wait_fixed(1),
    ...
    def test_stream_a_small_file():
        """This test streams a file into the real time api in the simplest way possible."""
        ...
        api.run_synchronously(audio_file, transcription_config, audio_settings, insecure=True)
