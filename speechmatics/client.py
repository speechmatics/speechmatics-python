...
class WebsocketClient:
    ...
    The best way to interact with this library is to instantiate this client and then add
    a set of handlers to it. Handlers respond to particular types of messages received from the
    server.

    Args:
        connection_settings (speechmatics.models.ConnectionSettings): Settings for the WebSocket
            connection, including the URL of the server.
    """
    ...
    def __init__(self, connection_settings):
        ...
        # The following asyncio fields are fully instantiated in _init_synchronization_primitives
        self._recognition_started = asyncio.Event
        # Semaphore used to ensure that we don't send too much audio data to the server too quickly
        # and burst any buffers downstream.
    ...
    def _set_recognition_config(self):
        ...
        call_middleware(self.middlewares, ClientMessageType.SetRecognitionConfig, msg, False)
        return msg
    ...
    def _start_recognition(self, audio_settings):
        ...
        Args:
            audio_settings (speechmatics.models.AudioSettings): Audio settings to use.
        """
        msg = {'message': ClientMessageType.StartRecognition,
               'audio_format': audio_settings.asdict(),
               'transcription_config': self.transcription_config.asdict()}
        self.session_running = True
        call_middleware(self.middlewares, ClientMessageType.StartRecognition, msg, False)
        LOGGER.debug(msg)
        return msg
    ...
    def _end_of_stream(self):
        ...
        """
        msg = {'message': ClientMessageType.EndOfStream,
               'last_seq_no': self.seq_no}
        call_middleware(self.middlewares, ClientMessageType.EndOfStream, msg, False)
        LOGGER.debug(msg)
        return msg
    ...
    async def _producer(self, stream, audio_chunk_size):
        ...
        async for audio_chunk in read_in_chunks(stream, audio_chunk_size):
            ...
            self.seq_no += 1
            call_middleware(self.middlewares, ClientMessageType.AddAudio, audio_chunk, True)
            yield audio_chunk
    ...
    def update_transcription_config(self, new_transcription_config):
        ...
        Args:
            new_transcription_config (speechmatics.models.TranscriptionConfig): The new
                config object.
        """
    ...
    def add_event_handler(self, event_name, event_handler):
        """
        Add an event handler (a.k.a. callback function) to handle an incoming message from the
        server. Event handlers are passed a copy of the incoming message from the server.
        If `event_name` is set to 'all' then the handler will be added for every event.

        For example, a simple handler that just prints out the AddTranscript messages received:

        >>> client = WebsocketClient(ConnectionSettings(url="wss://localhost:9000"))
        ...
        Args:
            event_name (str): The name of the message for which a handler is being added.
                Refer to the V2 API docs for a list of the possible message types.
            event_handler (Callable[[dict], None]): A function to be called when a message
                of the given type is received.
        ...
        if event_name == "all":
            ...
            raise ValueError(
                ...
                f"expected to be 'all' or one of {list(self.event_handlers.keys())}.")
        else:
    ...
    def add_middleware(self, event_name, middleware):
        ...
        Args:
            event_name (str): The name of the message for which a middleware is being added.
                Refer to the V2 API docs for a list of the possible message types.
            middleware (Callable[[dict, bool], None]): A function to be called to process an
                outgoing message of the given type. The function receives the message as the
                first argument and a second, boolean argument indicating whether or not the
                message is binary data (which implies it is an AddAudio message).
        ...
        if event_name == "all":
            ...
            raise ValueError(
                (f"Unknown event name: {event_name}, "
                 f"expected to be 'all' or one of {list(self.middlewares.keys())}."))
        else:
    ...
    async def run(self, stream, transcription_config, audio_settings):
        ...
        This will run asynchronously. Most callers may prefer to use run_synchronously which
        will block until the session is finished.

        Args:
            stream (io.IOBase): File-like object which an audio stream can be read from.
            transcription_config (speechmatics.models.TranscriptionConfig): Configuration
                for the transcription.
            audio_settings (speechmatics.models.AudioSettings): Configuration for the
                audio stream.
        ...
        try:
            ...
            LOGGER.error("Caught ConnectionResetError when attempting to connect to server. "
                         ...
                         "to use SSL but the server does not support SSL. If this is the case then "
                         "try using --ssl-mode=none")
            sys.exit(1)
        ...
        producer_task = asyncio.create_task(self._producer_handler(stream,
    ...
    def stop(self):
        ...
        You probably don't need to call this if you're running the client via `run_synchronously`.
        """
