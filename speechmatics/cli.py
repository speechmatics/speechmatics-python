...
def parse_additional_vocab(additional_vocab_filepath):
    ...
    Returns:
        List[Union[dict, str]]: A list of objects or strings which are the additional vocab items.
    ...
    with open(additional_vocab_filepath) as additional_vocab_file:
        try:
            ...
            raise SystemExit(
                f'Provided additional vocab at: {additional_vocab_filepath} is not valid json.')

        if not isinstance(additional_vocab, list):
            raise SystemExit((f'Additional vocab file at: {additional_vocab_filepath} '
                              'should be a list of objects/strings.'))

    if not additional_vocab:
        LOGGER.warning('Provided additional vocab at: %s is an empty list.',
...
def additional_vocab_item(to_parse):
    """
    Parses a single item of additional vocab. Used in conjunction with the additional vocab
    ...
    Returns:
        Union[dict, str]: Either a dictionary or a string depending on the form of the additional
            vocab item.
    ...
    if len(parts) > 2:
        raise argparse.ArgumentTypeError(
            f"Can't have more than one separator (:) in additional vocab: {to_parse}.")
...
def get_log_level(verbosity):
    ...
    try:
        ...
        key = int(str(error))
        raise SystemExit(
            f"Only supports 2 log levels eg. -vv, you are asking for -{'v' * key}")
...
def get_connection_settings(args):
    """
    Helper function which returns a ConnectionSettings object based on the command
    ...
    Returns:
        speechmatics.models.ConnectionSettings: Settings for the WebSocket connection.
    """
    settings = ConnectionSettings(url=args['url'], message_buffer_size=args["buffer_size"])
...
def get_transcription_config(args):
    """
    Helper function which returns a TranscriptionConfig object based on the command
    line options given to the program.
    ...
    if args["additional_vocab_file"]:
        ...
        LOGGER.info('Using additional vocab from file %s', args["additional_vocab_file"])
    ...
    if args["punctuation_permitted_marks"] is not None or args["punctuation_sensitivity"]:
        config.punctuation_overrides = {}

        if args["punctuation_permitted_marks"] is not None:
            config.punctuation_overrides["permitted_marks"] = (
                args["punctuation_permitted_marks"].split())

        if args["punctuation_sensitivity"]:
            config.punctuation_overrides["sensitivity"] = (
                args["punctuation_sensitivity"])
...
def get_audio_settings(args):
    ...
    Returns:
        speechmatics.models.TranscriptionConfig: Settings for the audio stream in the connection.
    """
...
def add_printing_handlers(api, transcripts, enable_partials=False, debug_handlers_too=False,
    ...
    Args:
        ...
        debug_handlers_too (bool, optional): Whether or not to enable 'debug' handlers that
            print out an ASCII symbol representing messages being received and sent.
        ...
        language (string, optional): The language code of the model being used. This is
            needed to configure language-specific text formatting.
    """
    if debug_handlers_too:
        api.add_event_handler(ServerMessageType.AudioAdded,
                              lambda *args: print_symbol("-"))
        api.add_event_handler(ServerMessageType.AddPartialTranscript,
                              lambda *args: print_symbol("."))
        api.add_event_handler(ServerMessageType.AddTranscript,
                              lambda *args: print_symbol("|"))
        api.add_middleware(ClientMessageType.AddAudio,
                           lambda *args: print_symbol("+"))
    ...
    def transcript_handler(message):
        ...
        if n_best_results:
            ...
            for alternative in n_best_list:
                words_joined = join_words((word["content"] for word in alternative["words"]),
                                          language=language)
...
def join_words(words, language="en"):
    """
    Joins a list of words with a language specific separator. Because not all languages
    use the standard English white-space between words.
...
def main(args=None):
    ...
    Args:
        args (List[str]): command-line arguments; defaults to None in which case arguments will retrieved
                          from `sys.argv` (this is useful mainly for unit tests).
    """
    if not args:
    ...
    if args["url"].lower().startswith("ws://") and args["ssl_mode"] != "none":
        raise SystemExit(f"ssl_mode '{args['ssl_mode']}' is incompatible with protocol 'ws'."
                         "Use 'wss' instead.")
    if args["url"].lower().startswith("wss://") and args["ssl_mode"] == "none":
        raise SystemExit(f"ssl_mode 'none' is incompatible with protocol 'wss'. Use 'ws' instead.")

    transcripts = Transcripts(text="", json=[])
    add_printing_handlers(api, transcripts,
    ...
    def run(stream):
        try:
            api.run_synchronously(stream, get_transcription_config(args), get_audio_settings(args))
...
def parse_args(args=None):
    ...
    parser.add_argument('-v', dest='verbose', action='count', default=0,
                        help=('Set the log level for verbose logs. '
                              'The number of flags indicate the level, eg. '
                              '-v is INFO and -vv is DEBUG.'))
    parser.add_argument('--ssl-mode', default="regular", choices=["regular", "insecure", "none"],
                        help=('Use a preset configuration for the SSL context. With `regular` mode '
                              'a valid certificate is expected. With `insecure` mode a self signed '
                              'certificate is allowed. With `none` then SSL is not used.'
                              ))
    parser.add_argument('--buffer-size', default=512, type=int,
                        help=('Maximum number of messages to send before waiting for '
                              'acknowledgements from the server.'))
    parser.add_argument('--debug', default=False, action='store_true',
                        help=('Prints useful symbols to represent the messages on the wire. '
    ...
    parser.add_argument('--output-locale', metavar='LOCALE', type=str, default=None,
                        help='Locale of the output of transcripts. eg. en-US')
    parser.add_argument('--additional-vocab', nargs='*', type=additional_vocab_item,
                        help=('Space separated list of additional vocab. Expected format: '
                              '<content (required)>:<sounds like (optional)>,<anymore sounds like> '
                              'Simple vocab list example: "Speechmatics gnocchi". '
                              'Vocab list with sounds like example: "gnocchi:nokey,nochi".'))
    parser.add_argument('--additional-vocab-file', metavar='VOCAB_FILEPATH', type=str,
                        help='File with additional vocab in JSON format')
    parser.add_argument('--enable-partials', default=False, action='store_true')
    parser.add_argument('--punctuation-permitted-marks', type=str, default=None,
                        help=('Space separated list of permitted punctuation marks for advanced '
                              'punctuation.'))
    parser.add_argument('--punctuation-sensitivity', type=float,
                        help='Sensitivity level for advanced punctuation.')
    parser.add_argument('--diarization', choices=['none', 'speaker_change'],
                        help='Which type of diarization to use.')
    parser.add_argument('--speaker-change-sensitivity', type=float,
                        help='Sensitivity level for speaker change.')
    parser.add_argument('--speaker-change-token', default=False, action='store_true',
                        help='Shows a <sc> token at any point where a speaker change was detected.')
    parser.add_argument('--max-delay', type=float)
    parser.add_argument('--raw', metavar='ENCODING', type=str,
                        help=('Indicate that the input audio is raw, provide the encoding of this '
                              'raw audio, eg. pcm_f32le'))
    parser.add_argument('--sample-rate', type=int, default=44100)
    parser.add_argument('--chunk-size', type=int, default=1024 * 4)
    parser.add_argument('--n-best-limit', type=int, default=None,
                        help='Upper bound on the number of N-best alternatives to return for each'
                             ' final. If not specified, N-best output is disabled. Be aware that'
                             ' this option is not supported for all Speechmatics products.')
    parser.add_argument('files', metavar='FILEPATHS', type=str, nargs='+',
